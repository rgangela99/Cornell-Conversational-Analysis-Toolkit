...
from .hypergraph_o import Hypergraph_0


class HyperConvo_0(Transformer):
    """
    ...
    def fit_transform(self, corpus: Corpus) -> Corpus:
        ...
        """
        feats = HyperConvo_0.retrieve_feats(corpus,
    ...
    def _make_hypergraph(corpus: Optional[Corpus]=None,
                         ...
                         exclude_id: Hashable=None) -> Hypergraph_0:
        """
    ...
    def _degree_feats(uts: Optional[Dict[Hashable, Utterance]]=None,
        ...
        if G is None:
            G = HyperConvo_0._make_hypergraph(uts, exclude_id=exclude_id)
            ...
            for to_hyper in [False, True]:
                ...
                for stat, stat_func in stat_funcs.items():
                    stats["{}[outdegree over {}->{} {}responses]".format(stat,
                                                                         HyperConvo_0._node_type_name(from_hyper),
                                                                         HyperConvo_0._node_type_name(to_hyper),
    ...
    def _motif_feats(uts: Optional[Dict[Hashable, Utterance]]=None,
        ...
        assert uts is None or G is None
        if G is None:
            G = HyperConvo_0._make_hypergraph(uts=uts, exclude_id=exclude_id)
    ...
    def retrieve_feats(corpus: Corpus, prefix_len: int=10,
        ...
        for i, (root, thread) in enumerate(
            ...
            stats = {}
            G = HyperConvo_0._make_hypergraph(uts=thread)
            G_mid = HyperConvo_0._make_hypergraph(uts=thread, exclude_id=root)
            for k, v in HyperConvo_0._degree_feats(G=G).items(): stats[k] = v
            for k, v in HyperConvo_0._motif_feats(G=G).items(): stats[k] = v
            for k, v in HyperConvo_0._degree_feats(G=G_mid,
                                                   name_ext="mid-thread ").items(): stats[k] = v
            for k, v in HyperConvo_0._motif_feats(G=G_mid,
